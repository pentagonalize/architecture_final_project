...
# Script to generate variants of neural network code with different parameters
# Hidden nodes: 64, 128, 256
# Activation functions: sigmoid, ReLU, hyperbolic tan

# Create output directory if it doesn't exist
mkdir -p mnist_networks

# Function to generate neural network code with specific parameters
generate_nn_code() {
    local hidden_nodes="$1"
    local activation="$2"
    local activation_func="$3"
    local activation_derivative="$4"
    local filename="mnist_networks/nn_${hidden_nodes}_${activation}.c"
    
    echo "Generating neural network with ${hidden_nodes} hidden nodes and ${activation} activation..."
...
#define HIDDEN_NODES ${hidden_nodes} // Number of hidden nodes
...
int main(int argc, char *argv[])
{
    ...
    printf("  Activation Function: ${activation}\n");
    ...
    save_weights_biases("model_${hidden_nodes}_${activation}.bin");
...
echo "You can compile it with: gcc -o ${hidden_nodes}_${activation} $filename -lm"
}

# Define sigmoid activation function and its derivative
sigmoid_func="double sigmoid(double x)
{
    return 1.0 / (1.0 + exp(-x));
}"

sigmoid_derivative="delta2[i] *= hidden[i] * (1 - hidden[i]);"

# Define ReLU activation function and its derivative
relu_func="double relu(double x)
{
    return x > 0 ? x : 0;
}
...
relu_derivative="delta2[i] *= (hidden_raw[i] > 0) ? 1 : 0;"

# Define tanh activation function and its derivative
tanh_func="double tanh_activation(double x)
{
    return tanh(x);
}
...
tanh_derivative="delta2[i] *= (1 - hidden[i] * hidden[i]);"

echo "Generating neural network variants..."

# Generate all combinations of hidden nodes and activation functions
for hidden_nodes in 64 128 256; do
    # Sigmoid variant
    generate_nn_code "$hidden_nodes" "sigmoid" "$sigmoid_func" "$sigmoid_derivative"
    
    # ReLU variant
    generate_nn_code "$hidden_nodes" "relu" "$relu_func" "$relu_derivative"
    
    # Tanh variant
    generate_nn_code "$hidden_nodes" "tanh_activation" "$tanh_func" "$tanh_derivative"
done

echo "Done! Generated 9 neural network variants."
echo
echo "Example usage:"
echo "  # Compile a specific variant"
echo "  gcc -o nn_64_sigmoid nn_variants/nn_64_sigmoid.c -lm"
echo
echo "  # Run with default parameters"
echo "  ./nn_64_sigmoid"
echo
echo "  # Run with custom parameters"
echo "  ./nn_64_sigmoid --train 1000 --test 100 --epochs 5"
echo
echo "  # Show help"
echo "  ./nn_64_sigmoid --help"
